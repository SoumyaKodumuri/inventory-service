...
public class InventoryService {
	...
	public InventoryResponse createInventoryEntry(InventoryRequest inventoryRequest) {
        ...
        return mapInventoryResponse(inventory);
			...
			log.error("Error occurred while creating inventory entry for skucode : {}", inventoryRequest.getSkucode(), e.getMessage(), e);
...
//		}
//		
//		return inventory.getStatus().equals(status.AVAILABLE) && inventory.getQuantity()>0;
//	}
	
	
	
	public List<InventoryResponse> getAllInventoryByUserId(Long userid) {
		try {
		...
		//	throw new RuntimeException("Failed to fetch inventories for userid: " + userid, e);
		}
		return null;
	}
	...
	public List<Boolean> areItemsInStock(List<OrderDetailsDTO> orderDetailsDto){
		...
		for(OrderDetailsDTO orderdetails : orderDetailsDto ) {
			Boolean status = inventoryRepository.findInventoryByskucode(orderdetails.getSkucode().getStatus.equals(status.AVAILABLE));
